...
class BackupScheduler {
  ...
  /// 예약된 백업 실행 - 매개변수 버전
  Future<void> _performScheduledBackup([BuildContext? context]) async {
    try {
      ...
      if (result.success) {
        // 성공 처리 - context가 있을 때만 localized 메시지 사용
        if (context != null) {
          await _handleBackupSuccess(result, context);
        } else {
          await _handleBackupSuccessLegacy(result);
        }
        ...
        // 실패 처리 - context가 있을 때만 localized 메시지 사용
        final error = result.error ?? '알 수 없는 오류';
        if (context != null) {
          await _handleBackupFailure(error, context);
        } else {
          await _handleBackupFailureLegacy(error);
        }
      }

    } catch (e) {
      if (context != null) {
        await _handleBackupFailure(e.toString(), context);
      } else {
        await _handleBackupFailureLegacy(e.toString());
      }
    }
  }

  /// 백업 성공 처리
  Future<void> _handleBackupSuccess(BackupResult result, BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    await _showBackupNotification(
      title: l10n.backupCompletedTitle,
      body: l10n.backupCompletedBody(_formatFileSize(result.size ?? 0)),
      isSuccess: true,
    );
  }

  /// 백업 실패 처리
  Future<void> _handleBackupFailure(String error, BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    final l10n = AppLocalizations.of(context);
    ...
    if (failureCount >= 3) {
      ...
      await _showBackupNotification(
        title: l10n.backupScheduleSuspendedTitle,
        body: l10n.backupScheduleSuspendedBody,
        isSuccess: false,
      );
      await stopSchedule();
    } else {
  ...
  Future<BackupResult> performManualBackup({
    ...
    if (context != null) {
      ...
      if (result.success) {
        await _showBackupNotification(
          title: l10n.manualBackupCompletedTitle,
          body: l10n.manualBackupCompletedBody,
          isSuccess: true,
        );
      } else {
        await _showBackupNotification(
          title: l10n.manualBackupFailedTitle,
          body: l10n.manualBackupFailedBody,
          isSuccess: false,
        );
      }
    } else {
  ...
  /// Legacy 백업 실패 처리 (임시 구현)
  Future<void> _handleBackupFailureLegacy(String error) async {
    // 임시 구현 - 실제 로직은 _handleBackupFailure 메서드 사용
    debugPrint('❌ Legacy 백업 실패 처리 (구버전 호환): $error');
  }
...
class BackupStatus {
  ...
  String getStatusText(BuildContext context) {
    ...
    if (!autoBackupEnabled) {
      return l10n.autoBackupDisabledStatus;
    }

    if (failureCount >= 3) {
      return l10n.backupFailureStoppedStatus;
    }

    if (!isSchedulerRunning) {
      return l10n.schedulerStoppedStatus;
    }

    final timeUntil = timeUntilNextBackup;
    if (timeUntil == null) {
      return l10n.backupWaitingStatus;
    }
