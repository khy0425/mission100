...
class WorkoutResumptionService {
  ...
  static Future<bool> hasResumableWorkout() async {
    try {
      ...
      final incompleteSession =
          await WorkoutHistoryService.recoverIncompleteSession();
  ...
  static Future<void> recordResumptionStats({
    ...
    try {
      ...
      await prefs.setInt(
        'total_recovered_sets',
        totalRecoveredSets + recoveredSets,
      );
      await prefs.setInt(
        'total_recovered_reps',
        totalRecoveredReps + recoveredReps,
      );
      await prefs.setString('last_resumption_source', resumptionSource);
      await prefs.setString(
        'last_resumption_time',
        DateTime.now().toIso8601String(),
      );

      debugPrint(
        '📊 재개 통계 기록: $resumptionSource - $recoveredSets세트, $recoveredReps회',
      );
    } catch (e) {
...
class WorkoutResumptionData {
  ...
  WorkoutResumptionData({this.sharedPrefsData, this.sessionData});
