...
class AchievementService {
  ...
  static Future<int> _checkChallengeAchievements(Achievement achievement) async {
    ...
    switch (achievement.id) {
      ...
      case 'first_challenge_complete':
        // 첫 번째 챌린지 완료 여부
        final completedChallenges = prefs.getStringList('completed_challenge_ids') ?? [];
      ...
      case 'skill_master':
        ...
        return (completedChallenges.contains('single_session_power') ||
                completedChallenges.contains('single_session_beast')) ? 1 : 0;
      ...
      case 'challenge_streak_5':
        ...
        final completedTimes = prefs.getStringList('challenge_completion_times') ?? [];
      ...
      case 'all_challenge_types':
        ...
        if (completedChallenges.contains('daily_perfect_form')) completedTypes.add('dailyPerfect');
        if (completedChallenges.contains('weekly_perfect_schedule')) completedTypes.add('weeklyGoal');
        if (completedChallenges.contains('single_session_power') ||
            completedChallenges.contains('single_session_beast')) completedTypes.add('skillChallenge');
        if (completedChallenges.contains('three_day_sprint')) completedTypes.add('sprintChallenge');
        if (completedChallenges.contains('monday_motivation')) completedTypes.add('eventChallenge');
      ...
      case 'challenge_master':
        // 총 10개 챌린지 완료
        final completedChallenges = prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.length >= 10 ? 10 : completedChallenges.length;

      case 'challenge_legend':
        // 총 20개 챌린지 완료
        final completedChallenges = prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.length >= 20 ? 20 : completedChallenges.length;

      case 'multi_challenger':
        // 동시에 3개 챌린지 진행 (현재 활성 챌린지 수)
        final activeChallenges = prefs.getStringList('active_challenge_ids') ?? [];
  ...
  static Future<List<Achievement>> checkAchievements() async {
    try {
      ...
      return allAchievements.where((a) => a.isUnlocked && !a.isNotified).toList();
    } catch (e) {
