...
class WorkoutHistoryService {
  ...
  static Future<void> _upgradeDatabase(
    ...
    if (oldVersion < 3) {
      ...
      try {
        await db.execute(
          'ALTER TABLE $tableName ADD COLUMN duration INTEGER DEFAULT 10',
        );
      ...
      try {
        await db.execute(
          'ALTER TABLE $tableName ADD COLUMN pushupType TEXT DEFAULT \'Push-up\'',
        );
  ...
  static Future<void> saveSetProgress({
    ...
    try {
      ...
      final completedRepsList = (session['completedReps'] as String)
          .split(',')
          .map(int.parse)
          .toList();
  ...
  static Future<void> completeWorkoutSession(String sessionId) async {
    ...
    try {
      await db.update(
        sessionTableName,
        {'isCompleted': 1, 'lastUpdateTime': DateTime.now().toIso8601String()},
        where: 'id = ?',
        whereArgs: [sessionId],
      );
  ...
  static Future<Map<String, dynamic>?> recoverIncompleteSession() async {
    ...
    try {
      ...
      if (sessions.isNotEmpty) {
        ...
        debugPrint(
          '🔄 미완료 세션 발견: ${session['id']} (${session['workoutTitle']})',
        );
        ...
        if (backupData != null) {
          ...
          return {...session, 'backupData': backupData};
        }
  ...
  /// 세션 백업 저장
  static Future<void> _saveSessionBackup(
    String sessionId,
  ...
  /// 세션 백업 로드
  static Future<Map<String, dynamic>?> _loadSessionBackup(
    String sessionId,
  ) async {
    try {
  ...
  static Future<void> saveWorkoutHistory(WorkoutHistory history) async {
    ...
    debugPrint(
      '📊 운동 데이터: ${history.totalReps}회, 완료율 ${(history.completionRate * 100).toStringAsFixed(1)}%',
    );
    ...
    try {
      ...
      debugPrint(
        '🗄️ 저장 후 운동 기록 개수: $countAfter개 (${countAfter - countBefore > 0 ? '증가' : '동일'})',
      );
      ...
      if (savedWorkout != null) {
        debugPrint(
          '✅ 운동 기록 저장 성공 확인: ${savedWorkout.id} - ${savedWorkout.totalReps}회',
        );
      } else {
      ...
      debugPrint(
        '📞 달력 업데이트 콜백 호출 시작 (등록된 콜백 수: ${_onWorkoutSavedCallbacks.length}개)',
      );
  ...
  // 오늘 완료된 운동 기록 조회 (호환성 메소드)
  static Future<WorkoutHistory?> getTodayCompletedWorkout(
    String dateStr,
  ) async {
    final db = await database;
