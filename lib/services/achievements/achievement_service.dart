...
class AchievementService {
  ...
  static Future<Achievement?> unlockAchievement(String achievementId) async {
    ...
    if (updated.isNotEmpty) {
      ...
      try {
        ...
        try {
          // import '../data/cloud_sync_service.dart'; 추가 필요
  ...
  static Future<List<Achievement>> checkAndUpdateAchievements() async {
    final overallTimer = _startPerformanceTimer(
      'checkAndUpdateAchievements_full',
    );
    ...
    try {
        ...
        try {
          ...
          switch (achievement.type) {
            case AchievementType.first:
              currentValue = await _checkFirstAchievements(
                achievement,
                workouts,
              );
              break;
            ...
            case AchievementType.special:
              currentValue = await _checkSpecialAchievements(
                achievement,
                workouts,
              );
              break;
            ...
            case AchievementType.statistics:
              currentValue = await _checkStatisticsAchievements(
                achievement,
                workouts,
              );
              break;
          ...
          debugPrint(
            '🎯 업적 ${achievement.id}: 현재 값 = $currentValue, 목표 값 = ${achievement.targetValue}',
          );
      ...
      debugPrint(
        '✅ 업적 체크 완료: $processedAchievements개 처리, ${newlyUnlocked.length}개 새로 잠금 해제',
      );
  ...
  /// 배치 업데이트 실행
  static Future<void> _executeBatchUpdates(
        ...
        for (final update in updates) {
          ...
          if (cached != null) {
            final updated = cached.copyWith(
              currentValue: update['currentValue'] as int?,
            );
            _updateCache(updated);
          }
        }
    ...
    switch (achievement.id) {
      ...
      case 'first_50_pushups':
        ...
        for (final workout in workouts) {
          final totalReps = workout.completedReps.fold<int>(
            0,
            (sum, reps) => sum + reps,
          );
          if (totalReps >= 50) {
  ...
  static Future<void> _forceInitializeAchievements() async {
    try {
      ...
      debugPrint(
        '🚀 업적 강제 초기화 시작 - 총 ${PredefinedAchievements.all.length}개 업적',
      );
  ...
  // 챌린지 업적 체크
  static Future<int> _checkChallengeAchievements(
    Achievement achievement,
  ) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    switch (achievement.id) {
      // === 새로운 챌린지 업적들 ===
      case 'first_challenge_complete':
        // 첫 번째 챌린지 완료 여부
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.isNotEmpty ? 1 : 0;

      case 'daily_perfect_challenger':
        // 일일 완벽 자세 챌린지 완료 여부
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.contains('daily_perfect_form') ? 1 : 0;

      case 'skill_master':
        // 스킬 챌린지 완료 여부 (30개 또는 50개)
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return (completedChallenges.contains('single_session_power') ||
                completedChallenges.contains('single_session_beast'))
            ? 1
            : 0;

      case 'perfect_cycle_champion':
        // 완벽한 휴식 주기 챌린지 완료 여부
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.contains('perfect_rest_cycle') ? 1 : 0;

      case 'weekly_schedule_master':
        // 주간 완벽 스케줄 완료 여부
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.contains('weekly_perfect_schedule') ? 1 : 0;

      case 'monday_crusher':
        // 월요일 모티베이션 완료 여부
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.contains('monday_motivation') ? 1 : 0;

      case 'challenge_streak_5':
        // 연속 5개 챌린지 완료 (시간 순서대로 완료한 것들)
        final completedTimes =
            prefs.getStringList('challenge_completion_times') ?? [];
        return completedTimes.length >= 5 ? 5 : completedTimes.length;

      case 'all_challenge_types':
        // 모든 타입의 챌린지 완료 (dailyPerfect, weeklyGoal, skillChallenge, sprintChallenge, eventChallenge)
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
      ...
      case 'challenge_legend':
        ...
        final completedChallenges =
            prefs.getStringList('completed_challenge_ids') ?? [];
        return completedChallenges.length >= 20
            ? 20
            : completedChallenges.length;

      case 'multi_challenger':
        // 동시에 3개 챌린지 진행 (현재 활성 챌린지 수)
        final activeChallenges =
            prefs.getStringList('active_challenge_ids') ?? [];
        return activeChallenges.length >= 3 ? 3 : activeChallenges.length;

      case 'speed_challenger':
        // 24시간 내 챌린지 완료 (빠른 완료 기록)
        return prefs.getBool('has_24h_challenge_completion') == true ? 1 : 0;
      ...
      case 'challenge_100_cumulative':
        return prefs.getBool('challenge_100_cumulative_completed') == true
            ? 1
            : 0;

      case 'challenge_200_cumulative':
        return prefs.getBool('challenge_200_cumulative_completed') == true
            ? 1
            : 0;
  ...
  static Future<int> _checkStatisticsAchievements(
    ...
    switch (achievement.id) {
      ...
      case 'completion_rate_95':
        final totalCompletionRate = workouts.fold<double>(
          0,
    ...
    try {
      ...
      if (totalCount != expectedCount) {
        ...
        (validation['issues'] as List<String>).add(
          '업적 개수 불일치: 예상 $expectedCount개, 실제 $totalCount개',
        );
      }
      ...
      if (invalidProgress.isNotEmpty) {
        for (final row in invalidProgress) {
          (validation['issues'] as List<String>).add(
            '비정상적인 진행도: ${row['id']} (${row['currentValue']}/${row['targetValue']})',
          );
        }
      }
      ...
      if (invalidTypes.isNotEmpty) {
        for (final row in invalidTypes) {
          (validation['issues'] as List<String>).add(
            '잘못된 타입/레어도: ${row['id']} (${row['type']}, ${row['rarity']})',
          );
        }
      }
      ...
      debugPrint(
        '📊 총 업적: ${validation['stats']['totalCount']}/${validation['stats']['expectedCount']}',
      );
      debugPrint(
        '🔓 잠금 해제: ${validation['stats']['unlockedCount']}개 (${validation['stats']['completionRate']}%)',
      );
  ...
  static Future<bool> repairAchievementDatabase() async {
    try {
      ...
      await db.rawQuery(
        'UPDATE $tableName SET currentValue = 0 WHERE currentValue < 0',
      );
    ...
    try {
      ...
      for (final achievement in PredefinedAchievements.all) {
        try {
          ...
          debugPrint(
            '🔄 ${achievement.id}: $currentValue/${achievement.targetValue}',
          );
        } catch (e) {
  ...
  /// 운동 완료 기록 및 자동 업적 체크
  static Future<void> recordWorkoutCompleted(
    int totalReps,
    ...
    try {
      ...
      debugPrint(
        '✅ 운동 기록 저장 완료 - 총 ${totalWorkouts + 1}회, 총 ${totalPushups + totalReps}개',
      );
  ...
  static Future<List<Achievement>> checkAchievements() async {
    try {
      ...
      return allAchievements
          .where((a) => a.isUnlocked && !a.isNotified)
          .toList();
    } catch (e) {
