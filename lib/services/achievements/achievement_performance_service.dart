...
class AchievementPerformanceService {
  ...
  static void _logSlowOperation(String operation, Duration duration) {
    final message =
        'SLOW_OPERATION: $operation took ${duration.inMilliseconds}ms';
    _logPerformance('WARNING', message);
    debugPrint('⚠️ $message');
  }

  /// 작업 오류 로그
  static void _logOperationError(
    String operation,
    Duration duration,
    dynamic error,
  ) {
    final message =
        'OPERATION_ERROR: $operation failed after ${duration.inMilliseconds}ms - $error';
    ...
    _operationTimings.forEach((operation, timings) {
      if (timings.isNotEmpty) {
        ...
        operationStats[operation] = {
          ...
          'avgMs': (durations.reduce((a, b) => a + b) / count / 1000)
              .toStringAsFixed(2),
          ...
          'p95Ms': (durations[(count * 0.95).floor()] / 1000).toStringAsFixed(
            2,
          ),
        };
      }
    ...
    try {
      ...
      if (duration != null && duration.inMilliseconds > 100) {
        await AchievementLogger.logPerformanceIssue(
          ...
          duration,
          details: {'cacheKey': cacheKey, 'isHit': isHit},
        );
      }
    } catch (e) {
  ...
  static Future<void> trackDatabaseOperation(
    ...
    try {
      ...
      await AchievementLogger.logDatabaseOperation(
        ...
        duration: duration,
        error: error,
        details: {'operationCount': _operationCounts[operation], ...?details},
      );
