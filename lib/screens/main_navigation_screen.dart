...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  void _onItemTapped(int index) {
    ...
    if (index == 0) {
      // 홈 화면 선택 시 - didChangeDependencies에서 자동으로 새로고침됨
  ...
  Achievement? _createAchievementFromEvent(Map<String, dynamic> event) {
    try {
      ...
      final titleKey =
          event['titleKey'] as String? ?? 'achievementDefaultTitle';
      final descriptionKey =
          event['descriptionKey'] as String? ?? 'achievementDefaultDesc';
      final motivationKey =
          event['motivationKey'] as String? ?? 'achievementDefaultMotivation';
