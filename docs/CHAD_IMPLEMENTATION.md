...
- **[CHAD_GUIDE.md](./CHAD_GUIDE.md)** - 콘셉트 & 철학
- **[CHAD_PRODUCTION.md](./CHAD_PRODUCTION.md)** - 연출 스펙 & 디자인
- **[CHAD_ASSETS.md](./CHAD_ASSETS.md)** - 이미지 생성 가이드
...
1. **ChadStats 모델** - 스탯 시스템
2. **BrainjoltMeter** - 뇌절도 게이지
3. **ChadDialogue** - 대사 시스템
4. **이미지 생성** - 9단계 Chad 이미지
...
### 1. 단계적 구현
한 번에 모든 것을 구현하지 말고, 단계적으로:
1. 먼저 기본 모델 (ChadStats)
2. 그 다음 간단한 UI (BrainjoltMeter)
3. 마지막으로 복잡한 애니메이션

### 2. 테스트 데이터
개발 중에는 테스트 데이터 사용:
```dart
final testStats = ChadStats.fromWorkoutData(
  level: 6,
  streakDays: 35,
  completedMissions: 42,
  totalMinutes: 2100,
...
### 3. Hot Reload 활용
Flutter의 hot reload를 적극 활용하여 빠르게 반복 개발
